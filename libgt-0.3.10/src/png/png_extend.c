/*
 * $Id: png_extend.c 98 2010-12-01 13:19:04Z ahto.truu $
 *
 ...
 *
 *
 * This is an example of extending a timestamp embedded in a PNG image
 * using the GuardTime SDK.
 *
 * To compile the example under Linux, use a command along the lines of
 *    gcc png_extend.c -o png_extend -I/usr/local/gt/include
 ...
 *    cl.exe png_extend.c /MT /I C:\gt\include
 ...
 *    png_extend data_in_file data_out_file verifier_url
 * for example
 *    png_extend TestData.png TestData.png
...
int main(int argc, char* argv[])
{
	...
	/* Read arguments. */
	if (argc != 4) {
		printf("Usage: %s data_in_file data_out_file verifier_url\n", argv[0]);
		goto cleanup;
	}
	in_file = argv[1];
	out_file = argv[2];
	...
	if (res != GT_OK) {
		fprintf(stderr, "Cannot load input file %s: %d (%s)\n",
	...
	/* Extend the timestamp embedded in the file. */
	res = GTPNG_extendTimestamp(in_data, in_data_len, tsa_url, &out_data, &out_data_len);
	if (res != GT_OK) {
		fprintf(stderr, "GTPNG_extendTimestamp() failed: %d (%s)\n",
