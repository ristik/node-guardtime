/*
 * $Id: gt_http.c 98 2010-12-01 13:19:04Z ahto.truu $
 *
		...
		char **error)
{
	...
	if (!HttpSendRequestA(req, NULL, 0, request, request_length)) {
		res = map_impl(GetLastError());
		goto cleanup;
	}
...
int GTHTTP_getPublicationsFile(const char *url, GTPublicationsFile **pub)
{
	...
	res = GTHTTP_sendRequest(url, NULL, 0, &response, &response_length, NULL);
	if (res != GT_OK) {
		goto cleanup;
	}
