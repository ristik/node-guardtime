/*
 * $Id: gt_timestamp.c 77 2010-03-10 16:09:47Z ahto.truu $
 *
...
/* Verification helper. Extracts location identifier from the given
 * location hash chain and stores it in the given buffer. */
static int extractLocationIdentifier(
		const ASN1_OCTET_STRING *hash_chain,
		GT_UInt64 *location_identifier)
{
	...
	for ( ; ; ) {
		...
		if (p - hash_chain->data == hash_chain->length) {
			...
			loc.hasher = collectBits(bits, &num_bits, 1);
...
//*/
	...
	*location_identifier |= loc.national_cluster;
	*location_identifier <<= 16;
	*location_identifier |= loc.state_cluster;
	*location_identifier <<= 16;
	*location_identifier |= loc.local_cluster;
	*location_identifier <<= 16;
	*location_identifier |= loc.client_id;
	res = GT_OK;
		...
		const STACK_OF(X509_ATTRIBUTE) *attrs)
{
	...
	for (i = 0; i < tmp_count; ++i) {
		...
		if (attr->single) {
			...
			tmp_der_len = i2d_ASN1_SET(
					attr->value.set, NULL, (i2d_of_void*) i2d_ASN1_TYPE,
			...
			if (tmp_der_len >= 0) {
				...
				p = tmp_der;
				tmp_der_len = i2d_ASN1_SET(
						attr->value.set, &p, (i2d_of_void*) i2d_ASN1_TYPE,
						V_ASN1_SET, V_ASN1_UNIVERSAL, IS_SET);
			}
		}
		if (tmp_der_len < 0) {
		...
		int *count, char ***list, const GTReferences *references)
{
	...
	if (references == NULL || sk_num(references) == 0) {
	...
	tmp_list = GT_malloc(sizeof(char*) * sk_num(references));
	if (tmp_list == NULL) {
		res = GT_OUT_OF_MEMORY;
		goto cleanup;
	}
	tmp_count = sk_num(references);
	...
	for (i = 0; i < tmp_count; ++i) {
		ref = (ASN1_OCTET_STRING*) sk_value(references, i);
		...
		int parse_data)
{
	...
	if (timestamp->time_signature->pubReference != NULL &&
			sk_num(timestamp->time_signature->pubReference) > 0) {
	...
	tmp_res = extractLocationIdentifier(
			timestamp->time_signature->location, &location_identifier);
	switch (tmp_res) {
	case GT_OK:
		break;
	case GT_INVALID_LINKING_INFO:
	...
	tmp_info->implicit_data->location_id = location_identifier;
